...
- 将域名 `aqztjx.top` 解析到阿里云中国内地服务器或静态托管资源。
- 上传 `app/dist` 内的构建产物内容，而不是上传 `dist` 文件夹本身。
- 访问首页，确认页脚能看到 `鲁ICP备2026031639号`，且点击后跳转到 `https://beian.miit.gov.cn/`。
- 通过 `https://beian.mps.gov.cn/` 办理公安备案；公安备案数据码属于临时操作凭据，不写入仓库文件。
